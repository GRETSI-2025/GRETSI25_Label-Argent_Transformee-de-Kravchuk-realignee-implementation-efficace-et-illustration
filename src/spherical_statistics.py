...
from ssht_transform      import the_ssht_transform, the_new_transform
...
def empirical_F(signal,transform = 'SSHT',disp = False):

    """
    unbiased estimator of the empty space function F of a point process on the sphere
    """

    if transform == 'SSHT':
        Ts,thetas,phis  = the_ssht_transform(signal)
    elif transform == 'new':
        Ts,thetas,phis  = the_new_transform(signal)
    else:
        error('Unknown transform. Only SSHT and new transforms are implemented. Default choice is SSHT.')    
    
    N                   = len(signal)-1
    zt, zp              = the_zeros(Ts,N,thetas,phis)
...
def empirical_K(signal,transform = 'SSHT',disp = False):
